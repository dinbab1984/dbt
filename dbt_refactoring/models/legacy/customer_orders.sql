...
FROM {{ source('raw', 'jaffle_shop_orders') }} as Orders
...
left join {{ source('raw', 'jaffle_shop_customers') }} C on orders.USER_ID = C.ID ),
...
from {{ source('raw', 'jaffle_shop_customers') }} C 
left join {{ source('raw', 'jaffle_shop_orders') }} as Orders
